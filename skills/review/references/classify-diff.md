...
- TRIVIAL complexity: code quality reviewer only **by default** — this is the baseline when no explicit escalation applies. An explicit "deep quality" / Code-judo ask or `ultra`/`max` effort still fires the deep lenses (per the TRIVIAL rule above), and impact or security sensitivity can escalate the tier.
...
- This skill also owns the **deep-tier escalation predicate** — the decision of whether `ultra`/`max` effort or a "deep review" / "deep quality" ask applies. Downstream orchestration (the review SKILL Invocation, `local-review.md`, `pr-review.md`, `review-pr.md`) consumes the `Deep-tier escalation: YES/NO` output and routes on it; it does not re-derive the trigger set. Keep the phrase list in one place here so the predicate cannot drift between the classifier and the orchestrators that read it.
