...
5. Launch fresh reviewer subagents on `review`/`deep-review` per [review-plan.md](review-plan.md)'s reviewer-selection and iterate-to-threshold steps (2–4): pick the reviewer set by plan scope, auto-iterate to 8/10, then cold-read via `planning/references/finalize.md`; they return findings and scores, and the main thread updates the plan until material findings are resolved and the Action Gate says proceed.
...
For 3+ independent slices, treat the work as STANDARD and keep the main thread as a thin orchestrator. `PLAN.md` owns the slice table, dependencies, entrance/exit criteria, and acceptance checks. PROJECT.md records only the active slice/wave and next action. Load `rules/orchestration.md` only when deciding subagent batch size or reasoning effort.
