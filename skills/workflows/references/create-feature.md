...
7. Checkpoint, request `context_reset`, then implement one bounded slice or wave inline by default, or with one bounded implementation subagent when isolation or parallelism clearly helps; any subagent returns `Implementation Handoff` blocks only.
...
| PM scoping | Main thread by default; PM subagent only for broad scope | [skills/pm/references/create-feature-brief.md](../../pm/references/create-feature-brief.md), [plan-milestones.md](../../pm/references/plan-milestones.md) | Load only when scope, milestones, acceptance criteria, or rollout are non-trivial. Broad means multiple product surfaces, rollout or permissions decisions, or unclear acceptance criteria. Handoff: product constraints and acceptance criteria. |
...
| Review | `review-code` reviewer subagents | [skills/review/references/local-review.md](../../review/references/local-review.md) | Run after `verify` or equivalent pre-flight checks for each meaningful implementation slice or wave. Branch on the Review Gate status before starting another wave. |
| Feature validation | Main thread by default; QA subagent only for large independent scenario sets | [skills/qa/references/validate-feature.md](../../qa/references/validate-feature.md) | Load when user-visible behavior changed and app is runnable. Return pass/fail/blockers and evidence paths. |
