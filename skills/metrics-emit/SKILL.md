...
- `premium_tokens` — the subset of `total_tokens` spent on a premium route: any call routed to an `opus` or `fable` family model, or a Codex `sol` model (per `interfaces/model-routing.json`'s `providers`; classification is by family, so every point release of a premium family counts), as opposed to the `sonnet` control-plane default
