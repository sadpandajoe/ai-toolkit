...
Dual-run note: [`decompose-work`](references/decompose-work.md) and
[`plan-phase`](references/plan-phase.md) exist alongside `iterate-review`/
`finalize` today, for `MULTI_PHASE` work only — the former decomposes the
whole unit once, the latter plans one phase at a time against that
decomposition. Neither older file is retired yet; this umbrella's live
Composition Flow below does not route through either until create-feature's
hierarchical-planning commit wires them in.
