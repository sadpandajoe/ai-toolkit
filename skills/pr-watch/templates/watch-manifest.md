...
Local-only, never committed (same rule as PROJECT.md / PLAN.md). One manifest
per watched PR; the watch resumes from this file alone.
...
Ledger rows are append-only within a watch. Keep Iteration Log entries to one
line; details live in PROJECT.md or the dispatched `fix-ci`/`address-feedback`
runs' own records.
