...
4. **Headless layers reset for free.** A `/schedule` routine starts every run in a fresh session that resumes from WATCH.md — prefer it for long watches. `/loop` re-invokes within the same conversation and does not reset context, which is exactly why points 1–2 matter there: a no-delta `/loop` iteration should add only a heartbeat line to the thread.
...
- `/loop /watch-pr <pr>` — long-horizon, self-paced re-invocation; right for comment babysitting after CI is stable.
- A `/schedule` routine wrapping `/watch-pr <pr>` — headless cron babysitting.

In-session watches end at first CI-stable; suggest the loop/schedule layer for ongoing comment watch rather than idling in-session.
