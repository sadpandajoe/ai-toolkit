...
- Run the parent session on the workhorse family: `/model sonnet` (or
  `opusplan`) on Claude Code, Sol on Codex. Opus plans only COMPLEX work; Sol or
  Opus review independently; Fable is a read-only deep advisor.
