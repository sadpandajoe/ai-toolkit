...
**Status: dual-run.** Nothing currently requires this exact shape.
`skills/implement-change/SKILL.md`'s `## Implementation Handoff` block is one
existing example the future migration should converge toward; other skills'
ad hoc handoff prose, and `rules/orchestration.md`'s Subagent Batch Rules
("compact handoffs: status, evidence summary, blockers, verification,
residual risk, next-action implications"), remain authoritative until each
caller migrates.
...
- **Evidence/artifact pointer**: a path (`PROJECT.md`, `PLAN.md`, a diff, a
  log excerpt) the specialist reads for context, never a transcript dump of
  the orchestrator's own conversation. If the specialist needs more than a
